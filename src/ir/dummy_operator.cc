...
namespace ir {
  ...
  OperatorType DummyOperator::get_type() {
    return DUMMY_OP;
  }

  bool DummyOperator::mapOnly() {
    return true;
  }

  pair<uint64_t, uint64_t> DummyOperator::get_output_size(
    map<string, pair<uint64_t, uint64_t> >* rel_size) {
    vector<Relation*> relations = get_relations();
    if (relations.size() != 1) {
      LOG(ERROR) << "Dummy operator has more than one relation";
  ...
  OperatorInterface* DummyOperator::clone() {
    return new DummyOperator(get_input_dir(), get_relations(),
                             get_output_relation());
  }

  bool DummyOperator::isMPC() {
    return true;
  }
